...
public class SplashPermissionsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        Logger.debug("PogoEnhancerJ", "Splashpermission loading");
        /** Default creation code. */
        super.onCreate(savedInstanceState);
        Logger.debug("PogoEnhancerJ", "Placing layout_old");
        /** Create the layout_old that will hold the TextView. */
        LinearLayout mainLayout = new LinearLayout(this);
        mainLayout.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        Logger.info("PogoEnhancerJ", "Adding text view");
    ...
    private void startNextActivity() {
        ...
        boolean systemized = this.isSystemPackage();
        SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(this).edit();
        editor.putBoolean(Constants.SHAREDPERFERENCES_KEYS.APP_SYSTEMIZED, systemized);
        editor.apply();
    ...
    private void checkPermissions() {
        ...
        if (ungrantedPermissions.length == 0) {
            ...
            runOnUiThread(() -> textView.setText("Missing permissions: " + Arrays.toString(ungrantedPermissions)));
    ...
    private String[] requiredPermissionsStillNeeded() {
        ...
        for (String permission : getRequiredPermissions()) {
            if (!permission.equals("android.permission.SYSTEM_ALERT_WINDOW")
                    && !permission.equals("android.permission.FOREGROUND_SERVICE")
                    && !permission.equals("android.permission.REQUEST_INSTALL_PACKAGES")
                    && !permission.equals("android.permission.CLEAR_APP_CACHE")
                    && !permission.equals("android.permission.INSTALL_LOCATION_PROVIDER")
                    && !permission.equals("android.permission.LOCATION_HARDWARE")
                    && !permission.equals("android.permission.ACCESS_MOCK_LOCATION")
                    && !permission.equals("android.permission.QUERY_ALL_PACKAGES")) { //TODO: check why FOREGROUND_SERVICE is troubling
...
//                Logger.e(SplashPermissionsActivity.class.getSimpleName(),
//                        "Permission: " + permission + " already granted.");
                i.remove();
            } else {
                Logger.error("PogoEnhancerJ",
